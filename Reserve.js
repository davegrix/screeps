...
let reserve = {
    ...
    run: function () {
        ...
        let TransferCount = 1;
        let TransferLeftCount = 3;
        let TransferCounts = 0;
        let HarvesterCount = 1;
        let HarvesterLeftCount = 1;
        let RepairCount = 2;
        let Defender = 1;
        ...
        if (_.filter(Game.creeps, (creep) => creep.memory.role === "reserve").length < ReserverCount) {
            Game.spawns.Spawn1.createCreep([MOVE,MOVE,MOVE, CLAIM, CLAIM], undefined, {role: "reserve"});
        }
      ...
      if (_.filter(Game.creeps, (creep) => creep.memory.role === "harvestleft").length < HarvesterLeftCount) {
        Game.spawns["Spawn3"].createCreep([MOVE,MOVE,MOVE,MOVE,MOVE,WORK,WORK,WORK,WORK,WORK], undefined, {role: "harvestleft"});
      }
      ...
      creeps.forEach(function (creep) {
        ...
        if (_.filter(Game.creeps, (creep) => creep.memory.role === "repair").length < RepairCount) {
          Game.spawns["Spawn1"].createCreep([
              ...
              WORK,WORK,WORK,WORK,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY], undefined, {role: "repair"});
        }
        ...
        if (_.filter(Game.creeps, (creep) => creep.memory.role === "carry").length < TransferCount) {
          Game.spawns["Spawn1"].createCreep([MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,
                CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY], undefined, {role: "carry"});
        }
      ...
      if (_.filter(Game.creeps, (creep) => creep.memory.role === "carry0").length < TransferLeftCount) {
        Game.spawns["Spawn3"].createCreep([MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,
          CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY], undefined, {role: "carry0"});
      }
      ...
      creeps.forEach(function (creep) {
        ...
        if(creep.carry.energy === 0) {
          ...
          creep.transfer(Game.getObjectById("5964264943d0e249138f6006"), RESOURCE_ENERGY);
      ...
      creeps.forEach(function (creep) {
        ...
        if (bt === null) {
          ...
          if (creep.carry.energy === 0) {
            ...
            if (creep.withdraw(Game.getObjectById("5964264943d0e249138f6006"), RESOURCE_ENERGY) === ERR_NOT_IN_RANGE) {
...
function getenergy(creep) {
  ...
  if(de) {
    if(creep.pickup(de) === ERR_NOT_IN_RANGE) {
  ...
  } else if(Game.getObjectById("597117ebbb6e4550728dd548").store.energy > 0) {
      ...
      if (Game.getObjectById("5956d51ac208c96e968bf8a6").energy > 200) {
        creep.moveTo(Game.getObjectById("5956d51ac208c96e968bf8a6"));
        creep.withdraw(Game.getObjectById("5956d51ac208c96e968bf8a6"), RESOURCE_ENERGY);
      }
    }
}
